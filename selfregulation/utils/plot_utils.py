...
def plot_correlation(corr, size = 30 , dpi=300, ext='png', subset = None, plot_dir=None, title = True, fontsize='fontsize'):
    ...
    with sns.axes_style("white"):
        ...
        sns.heatmap(corr, ax=ax1, square=True, vmax=1, vmin=-1, annot=True,fmt='.2f', mask =mask,
    ...
    if title == True:
            ax1.set_title('Loadings Correlations',
                      weight='bold', y=1.05, fontsize=size*3)
    if plot_dir:
        filename = 'loadings_correlations_EFA_%s.%s' % (subset, ext)
