...
def get_selected_var_psych(data_raw, vars_to_use=None):
    if vars_to_use is None:
        vars_to_use = ('bis11_survey.total',
                       'psychopathology_aes_survey__covid.psych_aes',
                       'psychopathology_sds_survey__covid.psych_sds',
                       'psychopathology_eat26_survey__covid.psych_eat26',
                       'psychopathology_stai_survey__covid.psych_stai',
                       'psychopathology_ocir_survey__covid.psych_ocir',
                       'psychopathology_sms_survey__covid.psych_sms',
                       'psychopathology_lsas_survey__covid.psych_lsas_total',
                       'alcohol_drugs_survey_post__covid.psych_audit_post'
                      )
    return(data_raw[list(vars_to_use)])
...
def transform_pt_train_test(data_train,
                 data_test = None,
                 threshold =1,
                 drop_failed = True,
                 verbose = True,
...
def fit_transform_pt(data_train,
                     threshold =1,
                     drop_failed = True,
                     verbose = True):
    data = data_train.copy()
    ...
    if drop_failed:
        ...
        if verbose:
            ...
            print('*' * 70)
    data_pt.columns = [i + '.pt' for i in data_pt]
    return data_pt.sort_index(axis = 1), pt
...
def transform_pt(  pt,
    ...
    #transformer
    #pt = PowerTransformer(method='yeo-johnson', standardize=True)
    data_pt = pd.DataFrame(pt.transform(data), columns = data.columns, index= data.index)
    print('*'*70)
    print('Testing dataset')
    print('*'*70)
...
def check_age(df1, df2, min_val, max_val):
    age_check = df2 - df1
    subjects_inconsistent_age = list(age_check [(age_check > max_val) | (age_check< min_val) ].index)
    print('# of subjects declaring an age which does not match time elapsed between assessments:', len(subjects_inconsistent_age))
    print(subjects_inconsistent_age)
    return subjects_inconsistent_age

def impute_age(list_subjects, df_impute, df_baseline, age_gap):
    """
    Assume real age declared at baseline and 4 time elapsed
    """
    for s in list_subjects:
        df_impute['demo'].loc[s,'Age'] =  df_baseline['demo'].loc[ s , 'Age' ] + age_gap
    return(df_impute)

def check_demo_vars(list_vars, list_subjects, df1, df2 ):
    """
    Check other demographics variables on which info can't be inconsistent across time points
    """
    temp1  = df1['demo'].loc[list_subjects , :]
    temp2  = df2['demo'].loc[list_subjects , :]

    assert temp1['Race'].equals(temp2['Race']), 'Subjects also reporting different Race'
    assert temp1['Sex'].equals(temp2['Sex']), 'Subjects also reporting different Gender'


    temp1  = temp1[list_vars].astype(int)
    temp2  = temp2[list_vars].astype(int)
    diff = temp2 - temp1
    drop_subjects = list(diff.loc[(diff < 0).any(axis=1)].index)
    print('Drop these subjects as incosistent on at least one of the other variables:\n', drop_subjects)
