...
def metadata_replace_two_with_nan(md):
    LevelsOrig=md['Levels'].copy()
    NewLevels={}
    for l in LevelsOrig:
        if not l.find('2')>-1:
...
def metadata_text_questions(md, orig_new_levels): 
    NewLevels = {}
    for key,value in orig_new_levels.items():
        for val in value:
            if val in NewLevels:
                NewLevels[val].append(key)
            else:
                NewLevels[val] = [key]
    md['LevelsOrig'] = orig_new_levels
    md['Levels'] = NewLevels
    return md
