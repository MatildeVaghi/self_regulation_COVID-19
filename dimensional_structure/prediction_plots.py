...
from selfregulation.utils.result_utils import ( load_results, get_EFA)
...
def get_short_names_fingerprint():
    shortened_factors = {
        'Sensation Seeking': 'SS',
        'Mindfulness': 'MND',
        'Emotional Control': 'EMC',
        'Impulsivity': 'IMP',
        'Goal-Dir+ Mind': 'GD/MND',
...
def plot_prediction(predictions, insample_predictions, shuffled, insample_shuffled,
                    set_vars = None,  EFA=None, outcome= None, title = None, comparison_label=None,
                    target_order=None,  metric='R2', size=4.6,
                    dpi=300, filename=None):
    sns.set_style('white')
    basefont = max(size, 5)
    if set_vars == 'task':
        colors = [sns.color_palette('Purples_d', 4)[i] for i in [1,3]]
    elif set_vars == 'survey':
        colors = [sns.color_palette('Greens_d', 8)[i] for i in [1,3]]
    ...
    fig = plt.figure(figsize=figsize)
    # plot bars
    ind = np.arange(len(predictions))
    width=.25
    ax1 = fig.add_axes([0,0,1,.5])


    ax1.bar(ind, [i[1] for i in predictions], width,
                label='Cross-validated prediction', color=colors[0])
    ax1.bar(ind+width, [i[1] for i in insample_predictions], width,
                label='Insample prediction', color=colors[1])

    # plot shuffled values above
    ax1.bar(ind, [i[1] for i in shuffled], width,
             color='none', edgecolor=['dimgrey']*len(ind), #amended  edgecolors  was not working
            linewidth=size/10, linestyle='--', label=comparison_label)
    ax1.bar(ind + width, [i[1] for i in insample_shuffled], width,
            color='none', edgecolor=['dimgrey']*len(ind+width), #amended edgecolors  was not working
            linewidth=size/10, linestyle='--')

    ax1.set_xticks(np.arange(0,len(predictions))+width/2)
    ax1.set_xticklabels([i[0] for i in predictions], rotation=0, fontsize=10*1.4)
    ...
    ax1.annotate('Pre-onset COVID-19', xy=(1, +1.1), xycoords=trans, ha="center", va="top", size =20)
    ax1.plot([-.4,2.4],[+1.02,+1.02], color="k", transform=trans, clip_on=False)
    ax1.annotate('Post-onset COVID-19', xy=(4, +1.1), xycoords=trans, ha="center", va="top", size =20)
    ax1.plot([2.9, 5.5],[+1.02,+1.02], color="k", transform=trans, clip_on=False)

    # add a legend
    leg = ax1.legend(fontsize=basefont*1.4, bbox_to_anchor = (1.05, 0.6), framealpha=1,
                     frameon=True, handlelength=0, handletextpad=0)
    leg.get_frame().set_linewidth(size/10)
    beautify_legend(leg, colors[:2]+[shuffled_grey])
    # change y extents
    ylim = ax1.get_ylim()
    r2_max = max(max(predictions, key=lambda x: x[1])[1],
                 max(insample_predictions, key=lambda x: x[1])[1])
    ymax = r2_max*1.5
    ax1.set_ylim(ylim[0], ymax)
    # change yticks
    if ymax<.15:
        ...
        ax1.yaxis.set_major_locator(ticker.MultipleLocator(.05))
        ax1.set_yticks(np.append([0, .050, .10, .150], np.arange(.15, 1, .05)))
    # draw grid
    ax1.set_axisbelow(True)
    plt.grid(axis='y', linestyle='dotted', linewidth=size/6)
    plt.setp(list(ax1.spines.values()), linewidth=size/10)
...
def get_predictions(results_dir, name, outcome, metric ='R2'):
    ...
    for d, directory in enumerate(directories):
        ...
        base_name = name + '_EFA'+ str(c) + '_oblimin_psych_' + outcome  + '_prediction_ridge.pkl'
        base_name_shuffled = name + '_EFA'+ str(c) + '_oblimin_psych_'+ outcome  + '_prediction_ridge_shuffle.pkl'


        predictions = pd.read_pickle(path.join(results_dir, 'prediction_outputs', directory, base_name))
        shuffled_predictions = pd.read_pickle(path.join(results_dir, 'prediction_outputs', directory, base_name_shuffled))

        predictions = predictions['data']
        comparison_predictions  = shuffled_predictions['data']

        target_order = sorted(predictions.keys())
        ...
        for i, k in enumerate(target_order):
            # normalize r2s to significance
            R2s = [i[metric] for i in comparison_predictions[k]['scores_cv']] # number of R2s corresponds to number of shuffles
            R2_95 = np.percentile(R2s, 95) #compute 95 percentile
            shuffled_r2s.append((k,R2_95))
            # and insample
            R2s = [i[metric] for i in comparison_predictions[k]['scores_insample']]
            R2_95 = np.percentile(R2s, 95)
        ...
        r2s_both.extend(r2s)
        insample_r2s_both.extend(insample_r2s)

        r2s_both = [(i, k) if k==k else (i,0) for i, k in r2s_both]
        insample_r2s_both = [(i, k) if k==k else (i,0) for i, k in insample_r2s_both]

    return(r2s_both, insample_r2s_both, shuffled_r2s, insample_shuffled_r2s)
...
def visualize_importance(importance, ax, xticklabels=True, yticklabels=True,
                         ...
                         ymax=None, color='k'):
    importance_vars = importance[0]
    shortened_factors = get_short_names_fingerprint()
    importance_vars = [shortened_factors.get(v,v) for v in importance_vars]
    if importance[1] is not None:
        importance_vals = [abs(i) for i in importance[1]]
        plot_loadings(ax, importance_vals, kind='line', offset=.5,
...
def plot_fingerprint(results_dir, directory, outcome, name = None, size = 10,dpi=300,  plot_dir = None,
                    filename = None ):
    sns.set_style('white')
    label_importance=[]
    #shortened_factors= shortened_factors
    ...
    N = len(importances)
    pad = 0
    best_predictors = sorted(enumerate(target_order), key = lambda x: x[1][1])

    locs = [.3, .6 ,.9]
    label_importance = importances[best_predictors[0][0]]
    text = [(l, shortened_factors.get(l, None)) for l in label_importance[0]] # for abbeviations text
    if len([True for t in text if t[1] is not None]) > 0:
        pad = .05
        text_ax = fig.add_axes([.8,.56,.1,.34])
    ...
    #Plot #1
    label_importance = importances[best_predictors[0][0]]
    title = best_predictors[0][1]

    visualize_importance(label_importance, axes[-1], yticklabels=True,
                             xticklabels=True,
                             label_size=max(figsize[1]*1.5, 5),
                             label_scale=.22,
                             title=title,
    ...
    # 3nd top
    label_importance = importances[best_predictors[2][0]]
    title = best_predictors[2][1]
    axes.append(fig.add_axes([locs[2]-.2*ratio-pad,.56,.3*ratio,.3], projection='polar'))
    visualize_importance(label_importance, axes[-1], yticklabels=True,
                                 xticklabels=True,
                                 label_size=max(figsize[1]*1.5, 5),
                                 label_scale=.22,
