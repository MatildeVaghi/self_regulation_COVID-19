...
def plot_cross_within_prediction(prediction_loc, size=4.6,
    ...
    colors = [sns.color_palette('Purples',10)[9],
              sns.color_palette('Greens',10)[9],
              [.4,.4,.4],
              [.4,.4,.4]]
...
def plot_BIC(all_results, size=4.6, dpi=300, ext='png', plot_dir=None):
    ...
    """
    all_colors = [sns.color_palette('Purples',10)[7:9],
              sns.color_palette('Greens',10)[7:9],
              sns.color_palette('Oranges',10)[4:6],
    ...
    for i, results in enumerate([all_results[key] for key in ['task','survey', 'crisis']]):
        ax1 = axes[i]
...
def plot_corr_heatmap(all_data, survey_order, task_order, EFA=False, size=4.6,
                   dpi=300, ext='png', plot_dir=None):
    f = plt.figure(figsize=(size,size))
    ax = f.add_axes([.05,.05,.8,.8])
    cbar_ax = f.add_axes([.86,.1,.04,.7])
