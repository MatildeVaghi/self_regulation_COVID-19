...
def plot_factor_correlation(results, c, rotate='oblimin', title=True,
    ...
    with sns.plotting_context('notebook', font_scale=2) and sns.axes_style('white'):
        ...
        sns.heatmap(phi, ax=ax1, square=True, vmax=1, vmin=-1,center =0,
                    cbar_ax=cbar_ax, annot=True, annot_kws={"size": size/c*15},fmt=".2f",
                    cmap=sns.diverging_palette(240,10,as_cmap=True),
                    mask=mask)
        yticklabels = ax1.get_yticklabels()
        ax1.set_yticklabels(yticklabels, rotation=0, ha="right")
        #ax1.set_yticklabels(textwrap.fill(y.get_text(), 20) for y in ax1.get_yticklabels())

        ax1.set(xticklabels=[])
        #ax1.set_xticklabels(ax1.get_xticklabels(), rotation=90)
        if title == True:
            ax1.set_title('%s Factor Scores Correlations' % results.ID.split('_')[0].title(),
...
def plot_heatmap_factors(results, c, size=4.6, thresh=75, rotate='oblimin',
    ...
    #ax.set_yticklabels(loadings.index, fontsize=DV_fontsize, rotation=0) #this put wrong labelsss!!
...
def rename_factor_names(df, subset):

    if subset == 'task':
        df=df.rename(columns={"Perc /Resp": "Perception \n Response",
                              "Caution": "Caution",
                              "Strategic IP": "Strategic \n IP",
                              "Speeded IP": "Speeded \n IP" })

    elif subset == 'crisis':

        df=df.rename(columns={'General Anxiety'  :  'General \n Anxiety',
                              'COVID-19 Worries' :  'COVID-19 \n Worries',
                              'Changes Relationship': 'Changes \n Relationship',
                              'Economic Concern': 'Economic \n Concern',
                              'Sleep Hours': 'Sleep \n Hours',
                              'Media Usage':  'Media \n Usage',
                              'Negative Mood': 'Negative \n Mood',
                              'Physical Exercise': 'Physical \n Exercise',
                              'Sleep Time': 'Sleep \n time',
                              'Stress Life Changes' :'Stress \n Life Changes'})

    elif subset == 'survey':
         df=df.rename(columns={'Agreeableness':'Agreeableness',
                               'Emotional Control':'Emotional \n Control ',
                               'Goal-directed/Mindfulness':'Goal-Directed \n Mindfulness',
                               'Risk Perception':'Risk \n Perception',
                               'Reward Sensitivity':'Reward \n Sensitivity' ,
                               'Sensation Seeking':'Sensation \n Seeking',
                               'Social Risk Taking':'Social \n Risk-Taking',
                               'Ethical Risk Taking': 'Ethical \n Risk-Taking'})

    elif subset == 'psych':
         df=df.rename(columns={'Anxious-Depressed':'Anxious \n Depressed',
                               'Compulsive Intrusive Thoughts':'Compulsive \n Intrusive \n Thoughts',
                               'Social Withdrawal': 'Social \n Withdrawal'})


    return df

def format_short_loadings_names(variables, subset):
    ...
    datadir = os.path.join(basedir,'references')
    if subset == 'psych':
        datafile=os.path.join(datadir,'variable_name_psych_lookup.csv')
        names_vars=pd.read_csv(datafile,index_col=0)
        names_vars_dict = pd.Series(names_vars['Loading Name'].values,index=names_vars['Short Name']).to_dict()


    elif subset == 'task' or subset =='survey':
        datafile=os.path.join(datadir,'variable_name_lookup.csv')
        names_vars=pd.read_csv(datafile,index_col=0)
        names_vars_dict = pd.Series(names_vars['Loading Name'].values,index=names_vars.index).to_dict()

    elif subset =='crisis':
        datafile=os.path.join(datadir,'variable_name_crisis_lookup.csv')
        names_vars=pd.read_csv(datafile,index_col=0)
        names_vars_dict = pd.Series(names_vars['Loading Name'].values,index=names_vars.index).to_dict()
    ...
    for var in variables:
        var = var.replace('.pt', '')
        if var in names_vars_dict:
            var = names_vars_dict.get(var)
        else:
            var = var.capitalize()
            var = var.replace('non_decision', 'non-dec')
            ...
            #var = var.replace('.', ': ')
            var = var.replace('correctlayout', ' ')
            var = var.replace('crisis', ' ')
            var = var.replace(': pt', ' ')
            var = ' '.join(var.split('_'))
            var = var.replace('task', '')
            var = var.replace('selective', 'sel')
            var = var.replace('Two stage', 'Two step')
            var = var.replace('Cognitive', 'Cog')
            var = var.replace('survey', '')


        new_vars.append(var)
    return new_vars

def plot_short_factor_loading(results, c, subset, rotate = 'oblimin', plot_dir = None ):
    ref_colors = {'survey': sns.color_palette("Greens_d", 8),
              'task':   sns.color_palette('Purples', 8),
              'psych': sns.color_palette('Blues', 8),
              'crisis': sns.color_palette('Oranges', 16)}
    ...
    for sublist in [i[1] for i in grouping]:
        flattened_factor_order += sublist
    loadings = loadings.loc[flattened_factor_order]
    loadings = rename_factor_names(loadings, subset)
    #Alphabetical order
    factor_names = sorted(loadings.columns)

    fig, ax = plt.subplots(len(factor_names),1, figsize=(3,len(factor_names)*2),
    ...
    for i,f  in enumerate(factor_names):
        ...
        inds = abs(loadings[f]).argsort()
        temp = loadings[f][inds].tail(5)
        temp.index = format_short_loadings_names(temp.index, subset)


        if subset == 'survey':
            face_color = ref_colors[subset][i]
        else:
            face_color = ref_colors[subset][i+4]
        ax[i]=temp.plot(kind='barh', ax=ax[i], color=face_color, width =.9, fontsize = 14)
        ax[i].set_xlim(-1, 1)
        ...
        if i == len(factor_names)-1:
            ax[i].set_xlabel("Loading", fontsize = 16)

        ax[i].set_facecolor('#DBDCE7')
        ax[i].tick_params(axis='y', which='both', direction='out', length=4, left=True)

        ax2 = ax[i].twinx()
        ax2.set_ylabel(f,rotation=90, fontsize = 14)
        ax2.set_yticklabels('')
    ...
    if plot_dir:
        ...
        save_figure(fig, path.join(plot_dir, 'Highest_loadings_%s.png' % subset),
                                    {'dpi': 300,'transparent':False, 'bbox_inches' : 'tight'} )
        plt.close()


def plot_factor_df(results, subset,  DA = False, rotate='oblimin'):
    if DA:
        EFA = results.DA
    else:
        EFA = results.EFA
    ...
    loadings = loadings.loc[flattened_factor_order]
    loadings = rename_factor_names(loadings, subset)
    loadings = loadings.sort_index(axis=1)
    loadings.index = format_short_loadings_names(loadings.index, subset)
    ...
    cm =sns.diverging_palette(220,15,n=220)
    def color_cell(val):
