...
def run_prediction(predictors, targets, output_base='',
                   ...
                   binarize=False, name = None):
    if name == None:
        output_dir=os.path.join(output_base,'prediction_outputs/')
    else:
        output_dir=os.path.join(output_base,'prediction_outputs/', name)
...
def run_prediction_local(factor_scores ,
                   target_scores ,
                   raw_data ,
                   clustering ,
                   name,
        ...
        raw_data = raw_data[labels]

        if predicting_DVs:
            targets =  [('psych_DVs', target_scores)]
        else:
            targets = [('psych_factors', target_scores)]
        #if include_raw_demographics:
