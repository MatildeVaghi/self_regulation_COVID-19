...
class EFA_Analysis:
    ...
    def get_dimensionality(self, metrics=None, verbose=False, name = 'task'):
        ...
        Args
            Metrics: A list including a subset of the following strings:
                ...
                alternatively use master to specify factors ad hoc
        """
        if metrics is None:
            metrics = ['BIC']
        if 'BIC' in metrics:
        ...
        if 'fixed' in metrics and 'task' in name:
            BIC_c = 5
            self.results['c_metric-fixed'] = BIC_c
        if 'fixed' in metrics and 'survey' in name:
            BIC_c = 12
            self.results['c_metric-fixed'] = BIC_c
        if 'fixed' in metrics and 'psych' in name:
            BIC_c = 3
            self.results['c_metric-fixed'] = BIC_c
    ...
    def run(self, loading_thresh=None, rotate='oblimin',
            bootstrap=False, verbose=False, dimensionality ='optim', name = 'task'):
        ...
        if 'c_metric-BIC' not in self.results.keys():
            if 'optim' in dimensionality:
                if verbose: print('Determining Optimal Dimensionality')
                self.get_dimensionality(verbose=verbose)
            if 'fixed' in dimensionality:
                if verbose: print('Using apriori decided Dimensionality')
                self.get_dimensionality(metrics='fixed', verbose=verbose, name = name)
...
class Demographic_Analysis(EFA_Analysis):
    ...
    def __init__(self, data, residualize=True, residualize_vars=['Age', 'Sex'],
        ...
        if residualize:
            ...
            #print('Shape demographic data', data.shape)
            #print('Columns in demographic data', data.columns)
...
class Results(EFA_Analysis, HCA_Analysis):
    ...
    def __init__(self,
        ...
        if saved_obj_file:
            ...
            # load data
            self.data = file
            self.data_no_impute = file_no_impute
            self.demographics = file_demographics#get_demographics(data_subset='Data_master')
    ...
    def run_EFA_analysis(self, rotate='oblimin', bootstrap=False, verbose=False, dimensionality ='optim', name ='task'):
        if verbose:
            print('*'*79)
            print('Running EFA, rotate: %s' % rotate)
            print('*'*79)
        self.EFA.run(self.loading_thresh, rotate=rotate,
                     bootstrap=bootstrap, verbose=verbose, dimensionality=dimensionality, name = name)
