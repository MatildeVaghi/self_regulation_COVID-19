...
def get_data(row):
    ...
    if row['experiment_template'] == 'jspsych':
        ...
        #############temporary hack#########################################
        #tehre are formatting issues when the question
        #is marked as checkbox, add id and text manually
        keys = [key for key in data.keys()]
        keys.sort()
        for question in keys:
            curr_question = data[question]
            try:
                tester = curr_question['id']
            except KeyError:
                data[question].update({'id' : question})
                data[question].update({'text' : 'text of the question'})
        ##################################################################ç
        survey =  data.values()
        for i in survey:
